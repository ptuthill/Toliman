...
def FT_model_broadband(pupil, aperture, m2_obsc, chip_dim, wavels, weights, foc_length, pix_size, transform_size, q1, q2, polar=True, leakages=[]):
    ...
    if len(leakages) == 0:
        for wavel, weight in zip(wavels, weights):
            im_out += weight * FT_model(pupil, aperture, m2_obsc, chip_dim, wavel, foc_length, pix_size, transform_size, q1, q2, polar=True, leakage=0.0)
    else:
        for wavel, weight, leakage in zip(wavels, weights, leakages):
            im_out += weight * FT_model(pupil, aperture, m2_obsc, chip_dim, wavel, foc_length, pix_size, transform_size, q1, q2, polar=True, leakage=leakage)
